...
function checkBoxToggle(chkBox) {
    ...
    switch (chkBox["id"]) {
    case "chkBoxPushSubs":
        if (audiencePushSubs) frmProfile.chkBoxPushSubs.selectedKeys = ["1"];
        else frmProfile.chkBoxPushSubs.selectedKeys = [];
        break;
    case "chkBoxSmsSubs":
        if (audienceSmsSubs) frmProfile.chkBoxSmsSubs.selectedKeys = ["1"];
        else frmProfile.chkBoxSmsSubs.selectedKeys = [];
        break;
    case "chkBoxEmailSubs":
        if (audienceEmailSubs) frmProfile.chkBoxEmailSubs.selectedKeys = ["1"];
        else frmProfile.chkBoxEmailSubs.selectedKeys = [];
    }
}
...
function frmProfilePreShow() {
    ...
    //frmProfile.chkBoxPushSubs["selectedKeys"]=subs;
    if (audienceStatus) frmProfile.imageProfile.src = "profile_male_active.png";
    else frmProfile.imageProfile.src = "profile_male_unavailable.png";
    ...
    if (audienceStatus) frmEditProfile.listboxActivityStatus.selectedKey = "1";
    else frmEditProfile.listboxActivityStatus.selectedKey = "2";
}

function updateActivityStatus(list) {
    if (list["selectedKeys"][0] == "1") audienceStatus = true;
    else audienceStatus = false;
