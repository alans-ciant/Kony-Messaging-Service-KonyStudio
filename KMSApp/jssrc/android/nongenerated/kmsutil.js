    ...
    function asyncCallback(status, result) {
        ...
        if (status == 400) {
            if (result["opstatus"] == 0) {
                ...
                if (result["message"] != undefined) updateMessaageAlert("" + result["message"]);
...
function getAudience() {
    ...
    try {
        ...
        var connHandle = kony.net.invokeServiceAsync("https://mobilefabric-demo.messaging.konycloud.com:443/api/v1/audience/" + audienceID, inputParamTable, asyncCallback);
    } catch (err) {
        kony.print("\nexeption in invoking service---\n" + JSON.stringify(err));
        alert("Error" + err);
    }
...
function getAccessToken4() {
    ...
    try {
        ...
        var connHandle = kony.net.invokeServiceAsync("https://mobilefabric-demo.messaging.konycloud.com/service/oauth/accesstoken/", inputParamTable, asyncCallback);
    } catch (err) {
        kony.print("\nexeption in invoking service---\n" + JSON.stringify(err));
        alert("Error" + err);
        kony.application.dismissLoadingScreen();
...
function KMSunregCallback(status, result) {
    ...
    if (status == 400) {
        ...
        if ((tmp["statusCode"] == 200)) {
            ...
            alert(tmp["message"]);
        } else alert("Failed to unsubscribe from Kony Messaging Service!!" + tmp["message"]);
    }
}
